...
conditional_mean <- function(data, mean_var, category_var){
  ...
  if(is.na(category_column)){
    stop(paste(substitute(category_column), " was not found in dataset:", substitute(data)))
  }

  if(is.na(mean_column)){
    stop(paste(substitute(mean_column), " was not found in dataset:", substitute(data)))
  }
